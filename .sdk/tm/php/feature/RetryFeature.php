...
// OpenLibrarySearch SDK retry feature
...
class OpenLibrarySearchRetryFeature extends OpenLibrarySearchBaseFeature
{
    ...
    public function init(OpenLibrarySearchContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (OpenLibrarySearchContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(OpenLibrarySearchContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(OpenLibrarySearchContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
