...
// OpenLibrarySearch SDK pipeline test
//
...
{
    private static function utility(): OpenLibrarySearchUtility
    {
        return new OpenLibrarySearchUtility();
    }
    ...
    private static function ctx(array $over = []): OpenLibrarySearchContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenLibrarySearchContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenLibrarySearchOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenLibrarySearchError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenLibrarySearchOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenLibrarySearchMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenLibrarySearchOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenLibrarySearchMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenLibrarySearchOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenLibrarySearchMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenLibrarySearchSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenLibrarySearchMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenLibrarySearchResponse([]),
            'result' => new OpenLibrarySearchResult([]),
        ]);
        [, $err] = OpenLibrarySearchMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenLibrarySearchSpec([]),
            'response' => null,
            'result' => new OpenLibrarySearchResult([]),
        ]);
        [, $err] = OpenLibrarySearchMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenLibrarySearchSpec(['step' => 's']),
            'response' => new OpenLibrarySearchResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenLibrarySearchResult([]),
        ]);
        [, $err] = OpenLibrarySearchMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenLibrarySearchSpec(['step' => 's']),
            'response' => new OpenLibrarySearchResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenLibrarySearchResult([]),
        ]);
        [, $err] = OpenLibrarySearchMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenLibrarySearchSpec(['step' => 's']),
            'response' => new OpenLibrarySearchResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenLibrarySearchResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenLibrarySearchMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenLibrarySearchResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenLibrarySearchSpec([]),
            'response' => new OpenLibrarySearchResponse([]),
            'result' => new OpenLibrarySearchResult([]),
        ...
        [$got, $err] = OpenLibrarySearchMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenLibrarySearchResult([])]);
        [, $err] = OpenLibrarySearchMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenLibrarySearchSpec([]), 'result' => null]);
        [, $err] = OpenLibrarySearchMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenLibrarySearchSpec(['step' => 's']),
            'result' => new OpenLibrarySearchResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenLibrarySearchOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenLibrarySearchMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenLibrarySearchSpec(['step' => 's']),
            'result' => new OpenLibrarySearchResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenLibrarySearchOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenLibrarySearchMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenLibrarySearchResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenLibrarySearchSpec([]),
            'result' => new OpenLibrarySearchResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenLibrarySearchMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenLibrarySearchMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenLibrarySearchError('boom', 'boom');
        $utility->fetcher = function (OpenLibrarySearchContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenLibrarySearchSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenLibrarySearchMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenLibrarySearchContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenLibrarySearchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenLibrarySearchResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenLibrarySearchMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenLibrarySearchContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenLibrarySearchSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenLibrarySearchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenLibrarySearchResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenLibrarySearchSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenLibrarySearchMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenLibrarySearchMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenLibrarySearchMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenLibrarySearchResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenLibrarySearchDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenLibrarySearchResult(['ok' => false])]);
        $this->expectException(OpenLibrarySearchError::class);
        OpenLibrarySearchDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenLibrarySearchResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenLibrarySearchMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenLibrarySearchMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenLibrarySearchBaseFeature();
        $b = new OpenLibrarySearchBaseFeature();
        OpenLibrarySearchFeatureAdd::call($ctx, $a);
        OpenLibrarySearchFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenLibrarySearchBaseFeature
    {
        $f = new OpenLibrarySearchBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenLibrarySearchFeatureAdd::call($ctx, self::named_feature('a'));
        OpenLibrarySearchFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        OpenLibrarySearchFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        OpenLibrarySearchFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        OpenLibrarySearchFeatureAdd::call($ctx, $replace);
        ...
        OpenLibrarySearchFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenLibrarySearchContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenLibrarySearchSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenLibrarySearchPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenLibrarySearchPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenLibrarySearchResponse(['headers' => null]),
            'result' => new OpenLibrarySearchResult([]),
        ]);
        OpenLibrarySearchResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenLibrarySearchResultBody::call($ctx);
