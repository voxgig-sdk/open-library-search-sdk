...
// OpenLibrarySearch SDK feature test
//
...
class FtCtrl extends OpenLibrarySearchControl
{
...
class FtClient
{
    ...
    private OpenLibrarySearchUtility $utility;

    public function __construct(OpenLibrarySearchUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenLibrarySearchUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenLibrarySearchContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenLibrarySearchContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenLibrarySearchContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenLibrarySearchConfig::make_feature($name);
    ...
    private static function buildUrl(OpenLibrarySearchSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenLibrarySearchError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenLibrarySearchResponse($fetched);
        }
    ...
    private function fail(OpenLibrarySearchContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenLibrarySearchContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
