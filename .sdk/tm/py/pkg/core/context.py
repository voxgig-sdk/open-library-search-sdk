...
from projectname_sdk.core.control import OpenLibrarySearchControl
...
class OpenLibrarySearchContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = OpenLibrarySearchControl()
        ...
        if isinstance(sp, OpenLibrarySearchSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenLibrarySearchResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenLibrarySearchResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return OpenLibrarySearchOperation({})
    ...
    def make_error(self, code, msg):
        return OpenLibrarySearchError(code, msg, self)
