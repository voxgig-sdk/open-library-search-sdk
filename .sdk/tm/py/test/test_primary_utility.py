...
from projectname_sdk import OpenLibrarySearchSDK
from projectname_sdk.core.spec import OpenLibrarySearchSpec
from projectname_sdk.core.result import OpenLibrarySearchResult
from projectname_sdk.core.response import OpenLibrarySearchResponse
from projectname_sdk.core.operation import OpenLibrarySearchOperation
from projectname_sdk.core.error import OpenLibrarySearchError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import OpenLibrarySearchBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return OpenLibrarySearchError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except OpenLibrarySearchError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = OpenLibrarySearchResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = OpenLibrarySearchSDK.test(None, None)
        ...
        class TestInitFeature(OpenLibrarySearchBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(OpenLibrarySearchBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = OpenLibrarySearchSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = OpenLibrarySearchSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_fetch_def_basic(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OpenLibrarySearchSpec({
            "base": "http://localhost:8080",
            "prefix": "/api",
            "path": "items/{id}",
            "suffix": "",
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OpenLibrarySearchSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = OpenLibrarySearchSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = OpenLibrarySearchOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = OpenLibrarySearchSDK.test(None, setup_opts)
