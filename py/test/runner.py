# OpenLibrarySearch SDK test runner
...
class OpenLibrarySearchTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenLibrarySearchTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenLibrarySearchTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenLibrarySearchTestRunner.getenv("OPEN_LIBRARY_SEARCH_TEST_LIVE")
        override = OpenLibrarySearchTestRunner.getenv("OPEN_LIBRARY_SEARCH_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenLibrarySearchTestRunner.getenv(key)
        ...
        explain = OpenLibrarySearchTestRunner.getenv("OPEN_LIBRARY_SEARCH_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPEN_LIBRARY_SEARCH_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenLibrarySearchTestRunner._test_control is not None:
            return OpenLibrarySearchTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenLibrarySearchTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenLibrarySearchTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenLibrarySearchTestRunner.load_test_control()
...
def load_env_local():
    OpenLibrarySearchTestRunner.load_env_local()


def env_override(m):
    return OpenLibrarySearchTestRunner.env_override(m)


def entity_data(v):
    return OpenLibrarySearchTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenLibrarySearchTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenLibrarySearchTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenLibrarySearchTestRunner.load_test_control()


def live_client_options():
    return OpenLibrarySearchTestRunner.live_client_options()


def live_delay_ms():
    return OpenLibrarySearchTestRunner.live_delay_ms()
