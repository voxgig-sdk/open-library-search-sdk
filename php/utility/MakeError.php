...
class OpenLibrarySearchMakeError
{
    public static function call(?OpenLibrarySearchContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OpenLibrarySearchContext([], null);
        }
        $op = $ctx->op ?? new OpenLibrarySearchOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OpenLibrarySearchResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OpenLibrarySearchError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OpenLibrarySearchError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof OpenLibrarySearchError) {
