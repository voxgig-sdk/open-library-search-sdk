...
// OpenLibrarySearch SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenLibrarySearchOmni::makeStructRunner(
    ...
    {
        return OpenLibrarySearchOmni::objmodel(OpenLibrarySearchOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return OpenLibrarySearchOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return OpenLibrarySearchOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenLibrarySearchOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenLibrarySearchOmni::structfix($val, Struct::undef()))) {
