...
class SearchEntity
{
    ...
    {
        if ($args) {
            $this->_data = OpenLibrarySearchHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = OpenLibrarySearchHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws OpenLibrarySearchError on failure (item-5 convention).
     */
