...
// OpenLibrarySearch SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OpenLibrarySearchTestFeature extends OpenLibrarySearchBaseFeature
{
    ...
    public function init(OpenLibrarySearchContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OpenLibrarySearchContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OpenLibrarySearchParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OpenLibrarySearchContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OpenLibrarySearchContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = OpenLibrarySearchParam::call($ctx, $k);
