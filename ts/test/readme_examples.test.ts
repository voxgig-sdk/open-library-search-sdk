...
//     (\`new OpenLibrarySearchSDK(...)\` / \`OpenLibrarySearchSDK.test(...)\` become
//     \`OpenLibrarySearchSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'OpenLibrarySearchSDK'
...
const TEST_SEED = {"entity":{"author":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenLibrarySearchSDK(...)\` and any
// \`OpenLibrarySearchSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenLibrarySearchSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenLibrarySearchSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
