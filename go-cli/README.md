# open-library-search-cli

boru-driven command-line client **and** interactive REPL for the OpenLibrarySearch
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-library-search-cli)
make build

# 2. See usage (words, entities, env vars)
./open-library-search-cli --help

# 3. Provide credentials once, via the environment
export OPEN_LIBRARY_SEARCH_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./open-library-search-cli list author
./open-library-search-cli list search

# 5. Override the API base URL for a single call
OPEN_LIBRARY_SEARCH_BASE=https://api.example.com ./open-library-search-cli list author

# 6. No arguments -> interactive REPL
./open-library-search-cli
open-library-search> list author
open-library-search> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-library-search-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-library-search-cli list author
   ```
...
Run with no arguments to open a REPL (prompt `open-library-search>`). Each line is
evaluated as its own boru expression:

```text
$ ./open-library-search-cli
open-library-search> list author
open-library-search> /help
open-library-search> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-library-search-cli`. |
...
(`./open-library-search-cli <expr>`) and interactively (the REPL), and expressions compose the
