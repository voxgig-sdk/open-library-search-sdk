...
MCP server exposing the OpenLibrarySearch SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `open-library-search_list` | `entity`, optional `query` map | First page of records as JSON |
| `open-library-search_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o open-library-search-mcp ./...
```
...
After install, restart Claude Code; the `open-library-search_list` and
`open-library-search_load` tools become available in new sessions.
