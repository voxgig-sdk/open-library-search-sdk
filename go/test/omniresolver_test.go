...
import (
	...
	omni "github.com/voxgig-sdk/open-library-search-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenLibrarySearchError); is {
				out := map[string]any{
					"name":    "OpenLibrarySearchError",
...
func omniCtx(arg any, client *sdk.OpenLibrarySearchSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenLibrarySearchSDK); is {
			client = live
