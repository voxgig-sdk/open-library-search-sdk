...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OPENLIBRARYSEARCH_TEST_LIVE") == "TRUE" ||
		os.Getenv("OPENLIBRARYSEARCH_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OPENLIBRARYSEARCH_TEST_EXPLAIN"); explain != "" {
