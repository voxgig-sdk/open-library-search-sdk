...
func TestSearchEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPEN_LIBRARY_SEARCH_TEST_SEARCH_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func searchBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPEN_LIBRARY_SEARCH_TEST_SEARCH_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPEN_LIBRARY_SEARCH_TEST_SEARCH_ENTID"])
	...
	if env["OPEN_LIBRARY_SEARCH_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewOpenLibrarySearchSDK(core.ToMapAny(mergedOpts))
	}
