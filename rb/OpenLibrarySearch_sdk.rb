...
require_relative 'OpenLibrarySearch_types'
...
class OpenLibrarySearchSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenLibrarySearchHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenLibrarySearchHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenLibrarySearchFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenLibrarySearchUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenLibrarySearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenLibrarySearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenLibrarySearchHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => OpenLibrarySearchError.new(
        "#{op}_allow",
        "OpenLibrarySearchSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue OpenLibrarySearchError => err
    ...
    if fetched.is_a?(Hash)
      status = OpenLibrarySearchHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenLibrarySearchError.new(
        "graphql_error", "OpenLibrarySearchSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenLibrarySearchSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
