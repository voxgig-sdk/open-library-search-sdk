...
module OpenLibrarySearchUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenLibrarySearchResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenLibrarySearchResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenLibrarySearchResponse.new(fetched)
    else
