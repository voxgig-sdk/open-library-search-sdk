# OpenLibrarySearch Ruby SDK



The Ruby SDK for the OpenLibrarySearch API — an entity-oriented client using idiomatic Ruby conventions.
...
raise a `OpenLibrarySearchError` (a `StandardError` subclass), so wrap
...
| `work_count` | `Integer` | Number of works by this author |
...
| `cover_i` | `Integer` | Cover image ID |
| `edition_count` | `Integer` | Number of editions for this work |
| `editions` | `Hash` | Nested editions data when fields parameter includes editions |
...
The main module (`OpenLibrarySearch_sdk`) exports the SDK class
