# OpenLibrarySearch SDK Author entity
...
class AuthorEntity
  ...
  def data_set(args)
    if args
      @_data = OpenLibrarySearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = OpenLibrarySearchHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Array<Author>, Array] the matching Author items; raises OpenLibrarySearchError on failure
